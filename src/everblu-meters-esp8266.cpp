...
void onScheduled()
{
  ...
  // At the scheduled hour/min, take the reading
  if (ptm->tm_hour == UTC_SCHEDULED_HOUR && ptm->tm_min == UTC_SCHEDULED_MIN && ptm->tm_sec == 0) {
...
mqtt.subscribe("everblu/cyble/restart", [](const String& message) {
  ...
  delay(50);
  mqtt.publish("homeassistant/binary_sensor/water_meter_wifi_status/config", jsonDiscoveryStatus, true);
  delay(50);
